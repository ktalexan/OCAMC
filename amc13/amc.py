...
class amc(object):
    ...
    def __init__(self, cadname, scale, scalefactor, cadpath, prjpath, tpob=None, direction=None, tolerance = 2):
        """
        ...
        self.outpath = os.path.join(self.prjpath, self.cadname)
    ...
    def baseChecks(self):
        ...
        self.appendReport(self.agpmsg(1))
        self.appendReport(self.agpmsg(1))
        ...
        if self.maptype == 'Tract':
            ...
            self.checkParcelMaps()
        elif self.maptype == 'Record of Survey':
            self.checkRecordOfSurveyMaps()
            ...
            for row in cursor:
                ...
                # Else, if current feature is a curve:
                    ...
                    if radtangent == 'Tangent':
                        desc_grid = f'{preamp} to the beginning of a curve, concave {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius, self.tolerance):.2f} feet; Thence {self.bearing2word(bearing)} along said curve {self.truncate(arclength, self.tolerance):.2f} feet through a central angle of {self.dd2dms(delta)};{closing}'
                        desc_ground = f'{preamp} to the beginning of a curve, concave {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius/self.scalefactor, self.tolerance):.2f} feet; Thence {self.bearing2word(bearing)} along said curve {self.truncate(arclength/self.scalefactor, self.tolerance):.2f} feet through a central angle of {self.dd2dms(delta)};{closing}'
                        ...
                        desc_grid = f'{preamp} to the beginning of a compound curve concave {self.bearing2word(midbearing)} and having a radius of {self.truncate(radius, self.tolerance):.2f} feet; Thence {self.bearing2word(bearing)} along said curve {self.truncate(arclength, self.tolerance):.2f} feet through a central angle of {self.dd2dms(delta)};{closing}'
                        desc_ground = f'{preamp} to the beginning of a compound curve {self.bearing2word(midbearing)} and having a radius of {self.truncate(radius/self.scalefactor, self.tolerance):.2f} feet; Thence {self.bearing2word(bearing)} along said curve {self.truncate(arclength/self.scalefactor, self.tolerance):.2f} feet through a central angle of {self.dd2dms(delta)};{closing}'
                        ann_grid = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}  R={self.truncate(radius, self.tolerance):.2f}  L={self.truncate(arclength, self.tolerance):.2f}'
                        ann_ground = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}  R={self.truncate(radius/self.scalefactor, self.tolerance):.2f}  L={self.truncate(arclength/self.scalefactor, self.tolerance):.2f}'
                        annweb_grid = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}\nR={self.truncate(radius, self.tolerance):.2f}\nL={self.truncate(arclength, self.tolerance):.2f}'
                        annweb_ground = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}\nR={self.truncate(radius/self.scalefactor, self.tolerance):.2f}\nL={self.truncate(arclength/self.scalefactor, self.tolerance):.2f}'
                        ...
                        desc_grid = f'{preamp} to the beginning of a reverse curve concave {self.bearing2word(midbearing)} and having a radius of {self.truncate(radius, self.tolerance):.2f} feet; Thence {self.bearing2word(bearing)} along said curve {self.truncate(arclength, self.tolerance):.2f} feet through a central angle of {self.dd2dms(delta)};{closing}'
                        desc_ground = f'{preamp} to the beginning of a reverse curve concave {self.bearing2word(midbearing)} and having a radius of {self.truncate(radius/self.scalefactor, self.tolerance):.2f} feet; Thence {self.bearing2word(bearing)} along said curve {self.truncate(arclength/self.scalefactor, self.tolerance):.2f} feet through a central angle of {self.dd2dms(delta)};{closing}'
                        ann_grid = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}  R={self.truncate(radius, self.tolerance):.2f}  L={self.truncate(arclength, self.tolerance):.2f}'
                        ann_ground = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}  R={self.truncate(radius/self.scalefactor, self.tolerance):.2f}  L={self.truncate(arclength/self.scalefactor, self.tolerance):.2f}'
                        annweb_grid = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}\nR={self.truncate(radius, self.tolerance):.2f}\nL={self.truncate(arclength, self.tolerance):.2f}'
                        annweb_ground = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}\nR={self.truncate(radius/self.scalefactor, self.tolerance):.2f}\nL={self.truncate(arclength/self.scalefactor, self.tolerance):.2f}'
                        ...
                        desc_grid = f'{preamp} to the beginning of a non-tangent curve, concave {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius, self.tolerance):.2f} feet, a radial line to said beginning of curve bears {dbearing}; Thence {self.bearing2word(bearing)} along said curve {self.truncate(arclength, self.tolerance):.2f} feet through a central angle of {self.dd2dms(delta)};{closing}'
                        desc_ground = f'{preamp} to the beginning of a non-tangent curve, concave {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius/self.scalefactor, self.tolerance):.2f} feet, a radial line to said beginning of curve bears {dbearing}; Thence {self.bearing2word(bearing)} along said curve {self.truncate(arclength/self.scalefactor, self.tolerance):.2f} feet through a central angle of {self.dd2dms(delta)};{closing}'
                        ann_grid = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}  R={self.truncate(radius, self.tolerance):.2f}  L={self.truncate(arclength, self.tolerance):.2f}'
                        ann_ground = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}  R={self.truncate(radius/self.scalefactor, self.tolerance):.2f}  L={self.truncate(arclength/self.scalefactor, self.tolerance):.2f}'
                        annweb_grid = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}\nR={self.truncate(radius, self.tolerance):.2f}\nL={self.truncate(arclength, self.tolerance):.2f}'
                        annweb_ground = f'\N{GREEK CAPITAL LETTER DELTA}={self.dd2dms(delta)}\nR={self.truncate(radius/self.scalefactor, self.tolerance):.2f}\nL={self.truncate(arclength/self.scalefactor, self.tolerance):.2f}'
    ...
    def finalizeReport(self):
        ...
        img = os.path.join(self.prjpath, 'SealKH.png')
        template = os.path.join(self.prjpath, 'LDTemplate.docx')
        parcelid = 1
        self.docLegalDescription(template, fontName = 'Arial', fontSize=10, exhibitNo='"A"', seal = img)
    ...
    def checkRecordOfSurveyMaps(self):
    ...
    def getCourse(self):
        ...
        while len(self.course) < len(segments): # runs until the course includes all the line segment
            nextLine = self.getnext(self.course, segments) # calls the getnext function above and obtains the data of the next line
    ...
    def getnext(self, course, segments):
    ...
    def dd2dms(self, dd):
        ...
        #dms = f'{int(math.floor(decimaldegree))}\N{DEGREE SIGN}{int(math.floor(minutes))}\N{APOSTROPHE}{int(seconds)}\N{QUOTATION MARK}'
    ...
    def describeHorizControls(self):
        """AMC Class Function: Describe Horizontal Controls
        ...
        annotation2 = self.bd2label(hc2bearing, hc2distance)
        gannotation2 = self.bd2label(hc2bearing, ghc2distance)
        ...
        if firstjson['shapetype'] == 'Curve':
            ...
            if firstjson['radtangent'] == 'Tangent':
                predesc = f', to the beginning of a curve, concave {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius, self.tolerance):.2f} feet, a radial bearing to said beginning of curve bears {dbearing}'
                gpredesc = f', to the beginning of a curve, concave {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius/self.scalefactor, self.tolerance):.2f} feet, a radial bearing to said beginning of curve bears {dbearing}'

            elif firstjson['radtangent'] == 'Reverse':
                predesc = f', to the beginning of a reverse curve {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius, self.tolerance):.2f} feet, a radial bearing to said beginning of curve bears {dbearing}'
                gpredesc = f', to the beginning of a reverse curve {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius/self.scalefactor, self.tolerance):.2f} feet, a radial bearing to said beginning of curve bears {dbearing}'

            elif firstjson['radtangent'] == 'Compound':
                predesc = f', to the beginning of a compound curve {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius, self.tolerance):.2f} feet, a radial bearing to said beginning of curve bears {dbearing}'
                gpredesc = f', to the beginning of a compound curve {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius/self.scalefactor, self.tolerance):.2f} feet, a radial bearing to said beginning of curve bears {dbearing}'

            else:
                predesc = f', to the beginning of a non-tangent curve, concave {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius, self.tolerance):.2f} feet, a radial bearing to said beginning of curve bears {dbearing}'
                gpredesc = f', to the beginning of a non-tangent curve, concave {self.bearing2word(midbearing)}, and having a radius of {self.truncate(radius/self.scalefactor, self.tolerance):.2f} feet, a radial bearing to said beginning of curve bears {dbearing}'
    ...
    def bd2label(self, bearing, distance):
        """AMC Class Function: Format Labels for Bearing and Distance
    ...
    #---------- AMC Class Function: Creating Word Document with Legal Description ----------#
    #self.docLegalDescription(self, template, fontName = 'Arial', fontSize=10, exhibitNo='"A"', seal = img)
    def docLegalDescription(self, template, fontName = 'Arial', fontSize=10, exhibitNo='<Exhibit No>', seal=None, poid=1):
        """AMC Class Function: Legal Description Document
        Creating an Office Word document (.docx) containing the legal description.
        """
        ...
        # Document heading
        parHead = doc.add_paragraph(self.cadname)
        ...
        mapstring = self.jsonLegalDescription['Map']
        doc.add_paragraph(mapstring)

        # Main Paragraph
        preampstring = self.jsonLegalDescription['Ground']['Preamp']
        p = doc.add_paragraph()
        r1= p.add_run()
        r1font = r1.font
        r1font.bold = True
        ...
        if 'COMMENCING' in preampstring:
            ...
            if 'TRUE POINT OF BEGINNING' in part1:
                ...
                # Course description
                coursestring = self.jsonLegalDescription['Ground']['Course']
        ...
        # Epilogue
        areaSqFeet = int(self.jsonControls['Areas'][poid]['SquareFeet'])
        areaAcres = round(self.jsonControls['Areas'][poid]['Acres'], 3)
        doc.add_paragraph(f'Containing an area of {areaSqFeet:,} square feet, or {areaAcres:.3f} acres.')
        doc.add_paragraph()
        doc.add_paragraph(f'All values are expressed on ground values. To get the grid values multiply values by the scale factor of {self.scalefactor}')
        doc.add_paragraph()
        ...
        self.appendReport('Legal Description word document created and saved to disk: Reference.docx')
