...
cadname = arcpy.GetParameterAsText(0) # The name of the map (tract, parcel, or record of survey)
cadpath = arcpy.GetParameterAsText(1) # The path to the project directory
prjpath = arcpy.GetParameterAsText(2) # The workspace folder path of the project directory
scale = arcpy.GetParameterAsText(3) # The scale parameter, choice of 'grid' (default) or 'ground'
scalefactor = arcpy.GetParameterAsText(4) # The scale factor of the drawing, e.g., 0.99996770
tpob = arcpy.GetParameterAsText(5) # The true point of beginning (optional, default = None) - if provided, must be coordinates
direction = arcpy.GetParameterAsText(6) # The direction of the boundary course (optional, default = None) - choose 'clockwise' or 'counter-clockwise'
tolerance = arcpy.GetParameterAsText(7) # the tolerance of the decimal points when comparing (optional, default = 2)
seal = arcpy.GetParameterAsText(8) # the seal image (optional, default = 'SealKH.png') - must be located in the project's workspace
doctemplate = arcpy.GetParameterAsText(9) # the word template (optional, default = 'LDTemplate.docx') - must be located in the project's workspace
...
# Setting the working directory as the project path
os.chdir(prjpath)
...
poid = 1 # To be implemented later with poid > 1
...
# Create empty directionaries for the pair of lines (either direction from the point of beginning or TPOB) to be 
# selected, and the segments of multiline coordinates and OIDs from the boundary feature class in the geodatabase.
...
for i in course:
    ...
    appendReport(report, f"\t\tCourse OID: {course[i]['oid']}")
    appendReport(report, f"\t\tCourse start point: {course[i]['start']}")
    appendReport(report, f"\t\tCourse end point: {course[i]['end']}")
    appendReport(report, f"\t\tCourse reversal: {course[i]['reversed']}")
...
arcpy.SetParameter(10, jsonResponse)
