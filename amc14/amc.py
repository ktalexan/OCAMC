...
class amc(object):
    ...
    def __init__(self, cadpath, prjpath, outpath, cadname, scale, scalefactor, tpob=None, direction=None, tolerance=2):
        ...
        f.write("{:^80s}\n\n".format("Python Class Execution Date and Time: {}".format(self.now)))
        f.close()
            ...
            for row in cursor:
                ...
                # Else, if current feature is a curve:
                    ...
                    if radtangent == "Tangent":
                        ...
                        annweb_grid = "{}={}\nR={:.2f}\nL={:.2f}".format("\N{GREEK CAPITAL LETTER DELTA}", self.dd2dms(delta), self.truncate(radius, self.tolerance), self.truncate(arclength, self.tolerance))
                        annweb_ground = "{}={}\nR={:.2f}\nL={:.2f}".format("\N{GREEK CAPITAL LETTER DELTA}", self.dd2dms(delta), self.truncate(radius/self.scalefactor, self.tolerance), self.truncate(arclength/self.scalefactor, self.tolerance))
    ...
    def checkLayers(self):
        ...
        layers = []
        with arcpy.da.SearchCursor(os.path.join(self.cadname, "Polyline"), ["Layer"]) as cursor:
            for row in cursor:
        ...
        for i, lyr in enumerate(LayerChecks, start = 1):
            if lyr in layers:
                self.appendReport("\tCheck {} of {}: {} in CAD Drawing: Passed".format(i, len(LayerChecks), lyr))
                self.jsonChecks["LayerChecks"] = "Pass"
                checks.append(True)
            elif lyr not in layers:
                self.appendReport("\tCheck {} of {}: {} not in CAD Drawing: Failed".format(i, len(LayerChecks), lyr))
        ...
        if all(checks):
            ...
            for lyr in LayerChecks:
                if lyr is not "NORTH ARROW MISCELLANEOUS":
                    ...
                    arcpy.Select_analysis(os.path.join(self.cadname, "Polyline"), outfc, """Layer = '{}'""".format(lyr))
    ...
    def checkGPS(self):
        ...
        self.gpspoints = []
        with arcpy.da.SearchCursor(os.path.join(self.cadname, "Annotation"), ["RefName", "SHAPE@XY"]) as cursor:
            n = 0
            for row in cursor:
                if "GPS" in row[0]:
                    n += 1
    ...
    def checkPOB(self):
        ...
        # if TPOB coordinates are not provided
            ...
            with arcpy.da.SearchCursor(os.path.join(self.cadname, "Point"), ["Layer", "SHAPE@XY"]) as cursor:
                for row in cursor:
                    if "TRUE POINT OF BEGINNING" in row[0]:
                        if arcpy.Exists("TPOB") == False:
                            where_clause = """Layer LIKE '%TRUE POINT OF BEGINNING%'"""
                            arcpy.Select_analysis(os.path.join(self.cadname, "Point"), "TPOB", where_clause)
    ...
    def checkClosureCentroid(self):
        ...
        if boundaryparcels == 1:
            ...
            self.appendReport("\tObtaining the centroid coordinates for boundary polygon {}".format(poid))
            with arcpy.da.UpdateCursor("BoundaryArea", ["OID@", "SHAPE@", "CentroidX", "CentroidY", "AreaSqFeet", "AreaAcres"]) as cursor:
                for row in cursor:
                    oid = row[0]
            ...
            self.jsonControls["Areas"][poid]["SquareFeet"] = areaSqFeet
            self.jsonControls["Areas"][poid]["Acres"] = areaAcres
            self.appendReport("\tBoundary area closure for polygon {}: Passed\n".format(poid))
